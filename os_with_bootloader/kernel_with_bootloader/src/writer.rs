...
impl FrameBufferWriter {
  ...
  {
    self.x_pos = x.min(self.width() - 1);
    self.y_pos = y.min(self.height() - 1);
  }
  ...
  fn scroll_screen(&mut self)
  {
    ...
    let scroll_distance = line_height;
    ...
    unsafe 
    {
        core::ptr::copy(
            ...
            self.framebuffer.len() - (scroll_distance * self.info.stride)
        );
    }

    //clear the last lines
    for y in (self.height() - scroll_distance) .. self.height()
    {
    ...
    self.y_pos = self.height() - line_height;
 ...
 pub fn write_char(&mut self, c: char) { //wasn't public before, had to change that
      match c {
          '\n' => 
          {
            self.y_pos += CHAR_RASTER_HEIGHT.val() + LINE_SPACING;
          ...
          c => {
             ...
             //scroll if esceeding screen height
             if self.y_pos >=self.height()
             {
                self.scroll_screen();
             }
...
pub fn custom_macro(writer: &mut FrameBufferWriter, text: &str)
{
    ...
    while let Some(c) = chars.next()
    {
        match c
        {
            ...
            '\t' => 
            {
                for _ in 0 ..4 
                {
                    writer.write_char(' ') //a tab here is 4 spaces
                }
            }
            ...
            '\\' => match chars.next()
            {
                Some('c') => 
                {
                    current_colour = 0x26; // Hot pink
                    writer.colour = current_colour;
                }, //makes my text hot pink!!
                Some('r') => 
                {
                    current_colour = 0x0E; // Back to yellow
                    writer.colour = current_colour;
                }, //takes text back to original yellow colour
                Some(unknown) => 
                {writer.write_char(unknown);}, //for an unknown escape sequence
                None => break, //stop if escape sequence is incomplete
            },
            _=> {
                writer.colour = current_colour; // Ensure color is set for each character
                writer.write_char(c); //writing regular characters
            }
        }
    }
}
